...
const TOOLS = [
  {
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          semaines_absence_sans_impact:         { type: 'number', description: 'Nombre de semaines où le dirigeant peut s'absenter sans impact sur l'exploitation' },
          pct_decisions_deleguees:              { type: 'number', description: '% des décisions opérationnelles déléguées (0-100)' },
          ...
          pct_savoir_faire_documente:           { type: 'number', description: '% du savoir-faire métier documenté (0-100)' },
          pct_top1_client:                      { type: 'number', description: '% du CA réalisé avec le 1er client' },
          pct_top3_clients:                     { type: 'number', description: '% du CA réalisé avec les 3 premiers clients' },
          nb_clients_actifs:                    { type: 'number', description: 'Nombre de clients actifs sur 12 mois' },
          pct_ca_recurrent_contractuel:         { type: 'number', description: '% du CA sous contrat récurrent (abonnement, contrat-cadre, etc.) (0-100)' },
          marge_ebe_pct:                        { type: 'number', description: 'Marge EBE de l'entreprise en % du CA' },
          marge_ebe_mediane_secteur_pct:        { type: 'number', description: 'Marge EBE médiane du secteur en %' },
          croissance_ca_moyenne_pct:            { type: 'number', description: 'Croissance annuelle moyenne du CA sur 3 ans (%)' },
          gearing:                              { type: 'number', description: 'Ratio dette financière nette / EBE normatif' },
          tendance_marge:                       { type: 'string', enum: ['amelioration','stable','degradation'], description: 'Tendance de la marge EBE sur 3 ans' },
          a_un_n1_identifie:                    { type: 'boolean', description: 'L'entreprise a un N-1 identifié capable de remplacer le dirigeant' },
          turnover_annuel_pct:                  { type: 'number', description: 'Taux de turnover annuel du personnel (%)' },
          nb_hommes_cles_non_securises:         { type: 'number', description: 'Nombre d'hommes-clés sans clause de non-concurrence ni engagement' },
          pct_rh_formalisee:                    { type: 'number', description: '% de la politique RH formalisée (fiches de poste, entretiens, etc.) (0-100)' },
          pct_process_cles_documentes:          { type: 'number', description: '% des process clés documentés (0-100)' },
          a_manuel_onboarding:                  { type: 'boolean', description: 'L'entreprise dispose d'un manuel d'onboarding ou de procédures d'intégration' },
          nb_non_conformites_majeures:          { type: 'number', description: 'Nombre de non-conformités juridiques majeures identifiées' },
          nb_non_conformites_mineures:          { type: 'number', description: 'Nombre de non-conformités juridiques mineures' },
          a_litiges_non_provisionnes:           { type: 'boolean', description: 'Existence de litiges significatifs non provisionnés' },
          croissance_secteur_pct:               { type: 'number', description: 'Croissance annuelle du secteur d'activité (%)' },
        ...
        properties: {
          ...
          retraitements: {
            ...
            properties: {
              ...
              produits_exceptionnels_non_recurrents:   { type: 'number', description: 'Produits exceptionnels à déduire (cession d'actifs, etc.)' },
              charges_personnelles_reintegrees:     { type: 'number', description: 'Avantages en nature ou charges personnelles passés en frais d'entreprise' },
            },
          },
        },
      },
      ...
      parameters: {
        ...
        properties: {
          ...
          eligibilite_retraite: {
            type: 'object',
            description: 'Paramètres d'éligibilité à l'abattement départ retraite (500 k€)',
            properties: {
          ...
          eligibilite_apport_cession: {
            type: 'object',
            description: 'Paramètres d'éligibilité à l'apport-cession (art. 150-0 B ter)',
            properties: {
          ...
          eligibilite_dutreil: {
            type: 'object',
            description: 'Paramètres d'éligibilité au Pacte Dutreil (donation)',
            properties: {
