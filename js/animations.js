...
(function () {
  ...
  // ─────────────────────────────────────────────────────────
  // 1. CINÉMATIQUE D'OUVERTURE (première visite uniquement)
  // ─────────────────────────────────────────────────────────
  function runOpeningCinematic() {
    if (sessionStorage.getItem('ev-opened')) return;
    sessionStorage.setItem('ev-opened', '1');
    ...
    for (var i = 0; i < N; i++) {
      ...
      var r = 120 * dpr + Math.random() * 30 * dpr;
      pts.push({
        tx: Math.cos(b) * Math.cos(a) * r, ty: Math.sin(b) * r, tz: Math.cos(b) * Math.sin(a) * r,
        x: (Math.random() - 0.5) * W * 2, y: (Math.random() - 0.5) * H * 2, z: Math.random() * 500 - 250,
        r: 1 + Math.random() * 2,
      });
    }
    ...
    function loop() {
      ...
      var rotY = p * Math.PI * 0.6, cosR = Math.cos(rotY), sinR = Math.sin(rotY);
      var projected = [];
      for (var i = 0; i < N; i++) {
        var pt = pts[i];
        var fx = pt.x + (pt.tx - pt.x) * ep, fy = pt.y + (pt.ty - pt.y) * ep, fz = pt.z + (pt.tz - pt.z) * ep;
        var rx = fx * cosR - fz * sinR, rz = fx * sinR + fz * cosR;
        var sc = 600 / (600 + rz);
        projected.push({ x: mx + rx * sc, y: my + fy * sc, z: rz, r: pt.r * sc, alpha: Math.min(ep * 2, 1) * (0.4 + sc * 0.4) });
      }
      ...
      if (ep > 0.3) {
        ...
        for (var i = 0; i < projected.length; i++) {
          for (var j = i + 1; j < projected.length; j++) {
            ...
            var dist = Math.sqrt(dx * dx + dy * dy), maxD = 80 * dpr * la;
            if (dist < maxD) {
              cx.beginPath(); cx.moveTo(projected[i].x, projected[i].y); cx.lineTo(projected[j].x, projected[j].y);
              cx.strokeStyle = 'rgba(99,102,241,' + (la * 0.25 * (1 - dist / maxD)).toFixed(3) + ')';
      ...
      if (ep > 0.55) {
        var ga = (ep - 0.55) * 2.2;
        cx.beginPath(); cx.arc(mx, my, 180 * dpr * ga, 0, 6.283);
        cx.strokeStyle = 'rgba(59,130,246,' + (0.3 * (1 - ga)).toFixed(3) + ')'; cx.lineWidth = 2 * dpr; cx.stroke();
      }
      if (p >= 0.8) {
        var fo = (p - 0.8) / 0.2;
        cx.font = 'bold ' + (28 * dpr) + 'px Inter, sans-serif';
        cx.textAlign = 'center'; cx.fillStyle = 'rgba(240,240,248,' + fo.toFixed(3) + ')';
        cx.fillText('Evoluty', mx, my + 160 * dpr);
        cx.font = (13 * dpr) + 'px Inter, sans-serif';
        cx.fillStyle = 'rgba(78,137,232,' + fo.toFixed(3) + ')';
        cx.fillText('Croissance & Transmission d\'entreprise par l\'IA', mx, my + 188 * dpr);
      }
      ...
      if (p < 1) {
        ...
        function fadeOut() {
          fo2 += 0.06;
          overlay.style.opacity = String(1 - fo2);
          if (fo2 < 1) requestAnimationFrame(fadeOut);
  ...
  // ─────────────────────────────────────────────────────────
  // 2. CERVEAU STRATÉGIQUE 3D (hero, colonne droite)
  // ─────────────────────────────────────────────────────────
  function initBrain() {
  ...
  // ─────────────────────────────────────────────────────────
  // 3. CUBE ÉCLATÉ — Section "Le Constat"
  // ─────────────────────────────────────────────────────────
  function initCube() {
  ...
  function initOrbits() {
    ...
    function orbitsLoop() {
      ...
      cx.fillStyle='rgba(10,14,30,0.75)'; var bw1=130*dpr,bh1=30*dpr; cx.beginPath(); cx.roundRect(mx-bw1/2,ly1-bh1/2,bw1,bh1,6*dpr); cx.fill(); cx.strokeStyle='rgba(139,92,246,0.45)'; cx.lineWidth=1*dpr; cx.stroke();
      ...
      cx.fillStyle='rgba(10,14,30,0.75)'; var bw2=140*dpr,bh2=30*dpr; cx.beginPath(); cx.roundRect(mx-bw2/2,ly2-bh2/2,bw2,bh2,6*dpr); cx.fill(); cx.strokeStyle='rgba(59,130,246,0.45)'; cx.lineWidth=1*dpr; cx.stroke();
